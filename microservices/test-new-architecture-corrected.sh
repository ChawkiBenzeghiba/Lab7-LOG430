...
echo "=== Test de Performance - Nouvelle Architecture (Microservices) - CORRIGÉ ==="
echo ""
...
measure_performance() {
    ...
    echo "$service_name,$avg_time,$success_rate" >> /tmp/new_architecture_results_corrected.csv
}

# Créer le fichier de résultats
echo "Service,Temps_Moyen,Taux_Succès" > /tmp/new_architecture_results_corrected.csv
...
# Test 5: Service Stock direct (route correcte)
echo "5. Test Service Stock direct (stock-central)..."
measure_performance "/api/stock-central" "Stock_Direct" "http://localhost:3003"

# Test 6: Service Panier direct (Instance 1)
echo "6. Test Service Panier direct (Instance 1)..."
measure_performance "/api/panier/client/1" "Panier_Direct_1" "http://localhost:3006"

# Test 7: Service Commandes direct (route correcte)
echo "7. Test Service Commandes direct (client/1)..."
measure_performance "/api/commandes/client/1" "Commandes_Direct" "http://localhost:3007"

# Test 8: Service Stock via Gateway (route correcte)
echo "8. Test Service Stock via Gateway (stock-central)..."
measure_performance "/api/stock-central" "Stock_Via_Gateway" "$GATEWAY_URL"

echo "=== Résumé des Tests ==="
echo ""
echo "Résultats sauvegardés dans /tmp/new_architecture_results_corrected.csv"
echo ""
...
if [ -f /tmp/new_architecture_results_corrected.csv ]; then
    tail -n +2 /tmp/new_architecture_results_corrected.csv | while IFS=',' read -r service time success; do
